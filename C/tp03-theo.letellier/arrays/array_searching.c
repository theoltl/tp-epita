 ...
 */
size_t __array_bin_search(int array[], size_t left, size_t right, int x)
{    
    ...
    if (x < array[mid])
        return __array_bin_search(array, left, mid, x);
    else
    {
        if(x > array[mid])
            return __array_bin_search(array, mid+1, right, x);
        else 
            return mid;
    }
...
size_t array_bin_search(int array[], size_t len, int x){
    return __array_bin_search(array, 0, len, x);
}
